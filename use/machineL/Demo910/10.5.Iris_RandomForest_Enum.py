...
if __name__ == "__main__":
    ...
    plt.figure(figsize=(10, 9), facecolor='#FFFFFF')    #10x9的框格，白色底
    for i, pair in enumerate(feature_pairs):
        # 准备数据
        x = x_prime[:, pair]    #选择两个特征
        ...
        y = y.reshape(-1)
        c = np.count_nonzero(y_hat == y)    # 统计预测正确的个数，用来算准确率
    ...
    plt.subplots_adjust(top=0.92)   #留 0.08给title，在这儿不管用呀
